...
function PokemonCard({id, name, image, type, weight, height, stats, statsName}) {
    ...
    return (
        <div className="container">
            ...
            <div className="show">
                ...
                <div style = {{display:"flex", width:"100%"}}>
                    ...
                    <div className="base-stats">
                        <div>
                            {statsName.map((statsName) => (
                            <p className="stats"> {statsName} </p>
                            ))}
                        </div>
                        <div>
                            {stats.map((stats) => (
            ...
            </div> // anexar funcao para mudar de pagina router no onlcik // search bar     
            )}
            <div className="right"
             ...
             onMouseLeave={() => setIsShown(false)}>            
                ...
                <p>{name}</p>
                <img src={pokeball} alt="pokeball" style={{marginLeft:"auto", width:"40px"}}></img>
            </div>
            {modalIsOpen && (
                <Modal 
                id={id}
